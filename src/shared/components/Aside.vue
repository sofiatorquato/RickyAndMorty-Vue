...
<template>
  ...
  <aside :class="['flex flex-col w-80 h-[calc(100vh-110px)] bg-secondary p-6 gap-6 border-r border-terc shadow-lg overflow-y-auto z-50 ','fixed top-[110px] left-0 lg:static lg:h-full lg:translate-x-0', ui.menuOpen ? 'translate-x-0' : '-translate-x-full']">
    ...
    <section class="flex flex-col gap-2">
      ...
      <input id ="busca" v-model="store.pesquisa"  type="text" placeholder="Busca por nome..." class="bg-[#2a2d2e] text-text-selects p-3 rounded-lg border border-transparent focus:border-terc outline-none transition-all font-gill placeholder:opacity-30" >
    </section>
    ...
    <section class="flex flex-col gap-3">
      ...
      <div class="relative flex flex-col">
        <select v-model="store.statusSelecionado" class="bg-[#2a2d2e] text-text-selects p-3 rounded-lg border border-transparent focus:border-terc outline-none appearance-none cursor-pointer font-gill" >
      ...
      <div class="relative flex flex-col">
        <select v-model="store.generoSelecionado" class="bg-[#2a2d2e] text-text-selects p-3 rounded-lg border border-transparent focus:border-terc outline-none appearance-none cursor-pointer font-gill">
    ...
    <button class="w-full py-2 border border-red-900  rounded-lg text-red-600 font-audiowide hover:bg-red-700 hover:text-white transition-all active:scale-95" @click="store.limparFiltros">
      Limpar Filtros
    </button>
